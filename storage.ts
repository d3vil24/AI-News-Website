...
export async function findArticleBySlug(slug: string) {
  if (!hasSupabaseConfig()) {
    return readLocalArticles().find((item) => item.slug === slug);
  }
  ...
  if (error) throw error;
  return data ? mapDbToArticle(data as DbArticle) : undefined;
}

export async function upsertArticle(article: Article) {
  if (!hasSupabaseConfig()) {
    const items = readLocalArticles();
    const index = items.findIndex((item) => item.id === article.id || item.slug === article.slug);
